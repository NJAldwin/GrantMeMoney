...
{
    public class UiGrantMoneyPanel : UIPanel
    {
        ...
        public override void Start()
        {
            ...
            var dragHandleObject = new GameObject("GMM_UiGrantMoneyPanel_DragHandler");
            ...
            _applyButton.eventClick += (c, p) => TryGrant(InputAmount);
        ...
        {
            var initAmt = GrantMeMoney.Instance == null ? 1_000_000 : GrantMeMoney.Instance.GrantAmount;
            Dbg.Log($"Init grant amount: ₡{initAmt:N}");
            UpdateAmt(initAmt);
            
            Status = "How much do you need?";
        }
        ...
        private void TryGrant(string input)
        {
            try
            {
                Grant(Convert.ToInt32(decimal.Parse(input.Trim(), NumberStyles.Currency)));
            }
            catch (Exception e)
            {
                Dbg.Err("Could not increase money", e);
                Status = "Unable to set up grant.";
            }
        }
        ...
        private void Grant(int amount)
        {
            ...
            {
                GrantMeMoney.Instance.GrantMoney(amount);
                UpdateAmt(GrantMeMoney.Instance.GrantAmount);
                Status = $"Approved!  ₡{InputAmount} granted.";
            }
            else
            {
                Status = "Could not set up grant.";
            }
        }
    }
}
